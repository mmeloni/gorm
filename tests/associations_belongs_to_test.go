...
func TestBelongsToAssociation(t *testing.T) {
	...
	// Count
	AssertAssociationCount(t, user, "Company", 1, "", db)
	AssertAssociationCount(t, user, "Manager", 1, "", db)
	...
	AssertAssociationCount(t, user2, "Company", 1, "AfterAppend", db)
	AssertAssociationCount(t, user2, "Manager", 1, "AfterAppend", db)
	...
	AssertAssociationCount(t, user2, "Company", 1, "AfterReplace", db)
	AssertAssociationCount(t, user2, "Manager", 1, "AfterReplace", db)

	// Delete
	if err := db.Model(&user2).Association("Company").Delete(&Company{}); err != nil {
		t.Fatalf("Error happened when delete Company, got %v", err)
	}
	AssertAssociationCount(t, user2, "Company", 1, "after delete non-existing data", db)

	if err := db.Model(&user2).Association("Company").Delete(&company2); err != nil {
		t.Fatalf("Error happened when delete Company, got %v", err)
	}
	AssertAssociationCount(t, user2, "Company", 0, "after delete", db)

	if err := db.Model(&user2).Association("Manager").Delete(&User{}); err != nil {
		t.Fatalf("Error happened when delete Manager, got %v", err)
	}
	AssertAssociationCount(t, user2, "Manager", 1, "after delete non-existing data", db)

	if err := db.Model(&user2).Association("Manager").Delete(manager2); err != nil {
		t.Fatalf("Error happened when delete Manager, got %v", err)
	}
	AssertAssociationCount(t, user2, "Manager", 0, "after delete", db)
	...
	AssertAssociationCount(t, user2, "Company", 1, "after prepare data", db)
	AssertAssociationCount(t, user2, "Manager", 1, "after prepare data", db)
	...
	AssertAssociationCount(t, user2, "Company", 0, "after clear", db)
	AssertAssociationCount(t, user2, "Manager", 0, "after clear", db)
...
func TestBelongsToAssociationForSlice(t *testing.T) {
	...
	AssertAssociationCount(t, users, "Company", 3, "", db)
	AssertAssociationCount(t, users, "Manager", 2, "", db)
	...
	AssertAssociationCount(t, users, "Company", 3, "After Append", db)
	...
	AssertAssociationCount(t, users, "Manager", 3, "After Append", db)
	...
	AssertAssociationCount(t, users, "Company", 2, "After Delete", db)

	// Clear
	db.Model(&users).Association("Company").Clear()
	AssertAssociationCount(t, users, "Company", 0, "After Clear", db)

	db.Model(&users).Association("Manager").Clear()
	AssertAssociationCount(t, users, "Manager", 0, "After Clear", db)
	...
	AssertAssociationCount(t, users[0], "Company", 0, "After Delete", db)
	AssertAssociationCount(t, users[1], "Company", 1, "After other user Delete", db)
}
